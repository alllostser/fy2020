...
import com.neuedu.common.DemoHbRunner;
...
public class AlipayUtil {
    private static Log                  log = LogFactory.getLog(com.neuedu.utils.AlipayUtil.class);
    ...
    // 测试当面付2.0生成支付二维码
    public static ServerResponse trade_precreate(OrderVo orderVo) {
        // (必填) 商户网站订单系统中唯一订单号，64个字符以内，只能包含字母、数字、下划线，
        // 需保证商户系统端不能重复，建议通过数据库sequence生成，
        String outTradeNo = String.valueOf(orderVo.getOrderNo());

        // (必填) 订单标题，粗略描述用户的支付目的。如“xxx品牌xxx门店当面付扫码消费”
        String subject = "商城门店当面付扫码消费";
        ...
        String body = "购买商品"+orderVo.getOrderItemVoList().size()+"件共"+totalAmount+"元";
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                return ServerResponse.serverResponseBySucess("支付宝预下单成功!!!");

            case FAILED:
                log.error("支付宝预下单失败!!!");
                return ServerResponse.serverResponseByFail(801,"支付宝预下单失败!!!");

            case UNKNOWN:
                log.error("系统异常，预下单状态未知!!!");
                return ServerResponse.serverResponseByFail(802,"系统异常，预下单状态未知!!!");

            default:
                log.error("不支持的交易状态，交易返回异常!!!");
                return ServerResponse.serverResponseByFail(802,"不支持的交易状态，交易返回异常!!!");
